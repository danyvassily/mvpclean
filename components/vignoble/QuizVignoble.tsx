  ...
  return (
    <DialogPrimitive.Root open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content className="fixed top-[50%] left-[50%] z-50 grid w-[calc(100%-2rem)] sm:w-[calc(100%-4rem)] max-w-lg max-h-[85vh] sm:max-h-[80vh] overflow-y-auto translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border border-slate-200 bg-white p-4 sm:p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">
          <DialogPrimitive.Title className="text-lg sm:text-xl md:text-2xl font-serif text-slate-900 text-center pb-2 sm:pb-4 leading-tight">
            {title}
        ...
        <div className="w-full h-2 bg-slate-200 rounded-full mb-4 sm:mb-6">
          <div
        ...
        <div className="space-y-4 sm:space-y-6">
          {!showResults ? (
            <>
              <div className="question">
                <h3 className="text-base sm:text-lg md:text-xl font-medium text-slate-900 mb-4 sm:mb-6 leading-snug">
              ...
              <ul className="space-y-2 sm:space-y-3">
                {questions[currentQuestion].options.map((option, index) => (
                  <li key={index}>
                    <label
                      className={cn(
                        "block cursor-pointer p-3 sm:p-4 rounded-lg border-2 transition-all duration-200 text-sm sm:text-base",
              ...
              <div className="flex justify-end pt-3 sm:pt-4">
                <button
                  ...
                  className={cn(
                    "px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg font-medium transition-all duration-200 text-sm sm:text-base text-white",
                    selectedAnswer === null
            ...
            <div className="text-center space-y-4 sm:space-y-6 py-4 sm:py-8">
              ...
              <button
                ...
                className="px-6 sm:px-8 py-3 sm:py-4 bg-slate-100 hover:bg-slate-200 border border-slate-300 hover:border-slate-400 text-slate-900 font-medium text-base sm:text-lg tracking-wide transition-all duration-300 rounded-full"
              >
                Rejouer le quiz
              </button>
            </div>
          ...
          <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-slate-200">
            <p className="text-xs sm:text-sm text-slate-600 text-center leading-relaxed px-2">
              {footerText}
            </p>
          </div>
